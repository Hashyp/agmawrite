...
/// The number of a Markdown heading level, indexing the palette's heading
/// roles shared with the write-mode highlighter.
fn heading_level(level: &markdown::HeadingLevel) -> usize {
    match level {
...
impl<'a> markdown::Viewer<'a, Message> for PreviewViewer<'a> {
    ...
    fn heading(
        ...
        self.text_element(
            settings,
            text,
            self.palette.heading_color(heading_level(level)),
        )
    }
