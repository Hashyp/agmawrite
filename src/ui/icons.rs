...
/// The opacity a disabled glyph draws with: visible, but clearly
/// inactive — the icon of an action that would do nothing.
...
/// foreground — so icons follow the palette like every other surface. A
/// dimmed icon keeps the same hue but fades it: it is disabled, publishing
/// nothing on click.
fn glyph_stroke(theme: &Theme, dimmed: bool) -> canvas::Stroke<'_> {
...
/// The open-document icon of the corner controls: a folder.
...
impl<Message> canvas::Program<Message> for OpenFileIcon {
    ...
    fn draw(
        ...
        frame.stroke(&folder, glyph_stroke(theme, self.dimmed));

        vec![frame.into_geometry()]
    }
}

/// The save-document icon of the corner controls: a floppy disk.
...
impl<Message> canvas::Program<Message> for SaveIcon {
    ...
    fn draw(
        ...
        let stroke = || glyph_stroke(theme, self.dimmed);
...
impl<Message> canvas::Program<Message> for CommentsIcon {
    ...
    fn draw(
        ...
        let stroke = || glyph_stroke(theme, false);
