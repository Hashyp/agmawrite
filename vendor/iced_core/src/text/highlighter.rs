...
pub trait Highlighter: 'static {
    ...
    /// Returns the height of the next source line as a multiplier of the
    /// editor's ordinary line height. The default keeps uniform spacing.
    ///
    /// Called immediately before [`Self::highlight_line`], with the same text
    /// and entering state. This must not advance the highlighter. Renderers
    /// ignore non-finite or non-positive scales. An override applies to every
    /// wrapped visual line of this source line, including empty lines.
    fn line_height_scale(&self, _line: &str) -> f32 {
        1.0
    }
