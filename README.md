...
The sidebar sits flush against the window's right edge, leaving the editor's margins symmetric; when it is hidden (`Ctrl + B`), a slim rail with a comment icon and the comment count marks where it collapsed to — click it to expand. A free text field with Add (`Ctrl + Enter`) and Publish (`Ctrl + Shift + Enter`) buttons sits at the bottom of the panel. Clicking a card activates its comment and moves the cursor to the element it was written for — the preview caret jumps there; in write mode the source cursor lands on the element's source. The sidebar appears once a comment exists and is cleared when a file is loaded.
